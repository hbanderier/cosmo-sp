...
# Start a new log
thisdir=`pwd`
echo "Merge log from $(date)" > $thisdir/merge_output_24h_plev.log
...
do
    ...
    for d in $dirs; do
		a=$(basename $d)
		if [[ $a == "24h_plev" ]]; then #"12h" "12h_plev" "24h" "24h_plev" "24h_100zlev"
	    	cd $d
           		...
           		for m in $(seq -f "%02g" 1 12); do
                	...
                	if ! ls lffdm${y}${m}* 1> /dev/null 2>&1; then
						...
						if compgen -G "lffd${datenextmonth}*" > /dev/null; then
                        	...
                        	echo "$d: $y$m File: $timefile Merge: $timemerge" >> $thisdir/merge_output_24h_plev.log
